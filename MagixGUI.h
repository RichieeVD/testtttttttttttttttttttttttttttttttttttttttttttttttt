...
protected:
	...
	MagixCritter* mSelectedCritter;

	// Emoji system
	std::map<OverlayElement*, Real> mActiveEmojis;
	bool mEmojisInitialized;
	OverlayElement* mEmojiTextArea;
	std::map<String, EmojiFontInfo> mEmojiFontMap;
	std::vector<String> mEmojiReplaceOrder;
...
public:
	...
	void cleanupEmojis();
	void loadEmojisFromConfig();
	bool isValidEmojiBoundary(const String& text, size_t pos, size_t length);
	void printAvailableEmojis();
	bool isEmojiSequence(const String& sequence);
	void addEmoji(const String& sequence, const String& fontName, const String& character);
	String processTextWithMixedFonts(const String& text);
	String replaceEmojisInText(const String& text);
	String processTextWithEmojis(const String& text);
	UTFString processEmojis(const UTFString &text);
	void updateEmojis(Real timeSinceLastFrame);
	void createEmojiInChat(const String &emojiName, Real x, Real y);
	String replaceEmojiWithTextures(const String &text);
	void renderEmojisInChat(const String &text, Real charHeight, Real boxWidth, Real boxLeft, Real boxTop);

	// Craft system
	void initializeCraftGUI();
	void showCraftGUI(MagixCritter* station = nullptr);
	void hideCraftGUI();
	void updateCraftText();
	void addItemToCraft(const String& itemName);
	void clearCraftItems();
	void onSelectCritter(MagixCritter* critter);
	void dropSelectedStashItem();
