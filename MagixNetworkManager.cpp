...
void MagixNetworkManager::update(const FrameEvent evt)
{
	...
	while (packet && tCont)
	{
		...
		case ID_WEATHER: processWeather(packet);break;
...
void MagixNetworkManager::processPlayerToken(Packet *p)
{
	...
	printf("=== PLAYER TOKEN RECEIVED: %i ===\n", mPlayerToken);
}
...
void MagixNetworkManager::processItemStash(Packet *p)
{
	...
	else
	{
		...
		if (tEquip != "")
		{
			if (tIsDrop)
			{
				// Дроп предмета на землю
				Vector3 playerPos = mUnitManager->getPlayer()->getPosition();
				const Real tX = playerPos.x + Math::RangeRandom(-2, 2);
				const Real tZ = playerPos.z + Math::RangeRandom(-2, 2);

				mUnitManager->createItem(mItemManager->getNextEmptyID(mUnitManager->getPlayer()->getIndex() * 100), tEquip, tX, tZ);
				mChatManager->message("Dropped " + mDef->getItemName(tEquip) + " from stash.");
			}
			else if (mUnitManager->getPlayer()->getEquip(tSlot) == "")
			{
				// Обычное извлечение в инвентарь
...
void MagixNetworkManager::sendWeather(const String &weatherType)
{
	if (!mPeer) return;

	OwnerToken token = mPlayerToken; 
	printf("=== SENDING WEATHER: %s, Token: %i ===\n", weatherType.c_str(), token);

	RakNet::BitStream tBitStream;
	tBitStream.Write(MessageID(ID_WEATHER));
	tBitStream.Write(token);
	StringCompressor::Instance()->EncodeString(weatherType.c_str(), 32, &tBitStream);

	mPeer->Send(&tBitStream, HIGH_PRIORITY, RELIABLE, 0, UNASSIGNED_SYSTEM_ADDRESS, true);
}
void MagixNetworkManager::processWeather(Packet *p)
{
	RakNet::BitStream tReceiveBit(p->data, p->length, false); // ← ИСПРАВЬТЕ НА tReceiveBit
	MessageID tMessage;
	OwnerToken tSender;
	char tWeatherType[32];

	tReceiveBit.Read(tMessage); // ← ИСПРАВЬТЕ
	tReceiveBit.Read(tSender); // ← ИСПРАВЬТЕ
	StringCompressor::Instance()->DecodeString(tWeatherType, 32, &tReceiveBit);

	printf("=== CLIENT: Received weather '%s' from sender %i ===\n", tWeatherType, tSender);

	String weatherType = tWeatherType;

	if (weatherType == "clear")
	{
		// Локально
		mSkyManager->destroyWeather();
		mSkyManager->setWeatherCycle("");
		if (mChatManager) mChatManager->message("Weather cleared");
	}
	else if (weatherType.find("cycle ") == 0)
	{
		String cycleName = weatherType.substr(6);
		// Локально
		mSkyManager->setWeatherCycle(cycleName);
		if (mChatManager) mChatManager->message("Weather cycle set to: " + cycleName);
	}
	else
	{
		String normalizedType = weatherType;
		if (!normalizedType.empty())
		{
			normalizedType[0] = std::toupper(normalizedType[0]);
			for (size_t i = 1; i < normalizedType.length(); ++i)
			{
				normalizedType[i] = std::tolower(normalizedType[i]);
			}
		}

		if (mChatManager) mChatManager->message("Weather changed to: " + normalizedType);
		...
		if (normalizedType == "Rain") {
			particleSystem = "Weather/Rain";
			offset = Vector3(0, 400, 0);
		}
		else if (normalizedType == "Snow") {
			particleSystem = "Weather/Snow";
			offset = Vector3(-400, 700, -600);
		}
		else if (normalizedType == "Starfall") {
			particleSystem = "Weather/Starfall";
			offset = Vector3(800, 1200, 1200);
		}
		else if (normalizedType == "Fireworks") {
			particleSystem = "Weather/Fireworks";
			offset = Vector3(-1000, 500, -1000);
		}
		else {
			if (mChatManager) mChatManager->message("Unknown weather type: " + normalizedType);
			return;
		}

		// ПОЛУЧАЕМ SCENEMANAGER через объект игрока
		SceneManager* sceneMgr = 0;
		if (mUnitManager && mUnitManager->getPlayer())
		{
			sceneMgr = mUnitManager->getPlayer()->getObjectNode()->getCreator();
		}

		if (!sceneMgr)
		{
			printf("ERROR: Could not get SceneManager!\n");
			return;
		}

		// УДАЛЯЕМ СТАРУЮ СИСТЕМУ ПЕРЕД СОЗДАНИЕМ НОВОЙ
		if (sceneMgr->hasParticleSystem("WeatherSystem"))
		{
			sceneMgr->destroyParticleSystem("WeatherSystem");
		}

		// СОЗДАЕМ НАПРЯМУЮ
		ParticleSystem* weatherSystem = sceneMgr->createParticleSystem("WeatherSystem", particleSystem);
		if (weatherSystem)
		{
			weatherSystem->setEmitting(true);
			weatherSystem->setVisible(true);

			SceneNode* weatherNode = sceneMgr->getSceneNode("WeatherOffsetNode");
			if (weatherNode)
			{
				weatherNode->attachObject(weatherSystem);
				weatherNode->setPosition(offset);
...
void MagixNetworkManager::sendItemDropFromStash(const unsigned short &line)
{
	RakNet::BitStream tBitStream;

	tBitStream.Write(MessageID(ID_ITEMSTASH));
	tBitStream.Write(false); // unstash = false (это операция со сташем)
	tBitStream.Write(line);  // индекс в сташе
	tBitStream.Write((unsigned short)0); // слот (не важен для дропа)
	tBitStream.Write(true);  // tIsDrop = true (это дроп, а не извлечение)
