...
void MagixUnit::setMarkingColours(const ColourValue &bodyMark1, const ColourValue &bodyMark2, const ColourValue &bodyMark3,
...
{
	// ПРОСТАЯ ЗАГЛУШКА: устанавливаем только bodyMark1 как цвет всех маркировок
	// Это уберет ошибки компиляции

	if (mBodyEnt)
	{
		try
		{
			// Проходим по всем саб-энтети и ищем материалы с маркировками
			for (unsigned short i = 0; i < mBodyEnt->getNumSubEntities(); i++)
			{
				SubEntity* subEnt = mBodyEnt->getSubEntity(i);
				if (!subEnt) continue;

				const String& matName = subEnt->getMaterialName();
				if (matName.find("Mark") != String::npos)
				{
					// Нашли материал маркировки - меняем его диффузный цвет
					MaterialPtr mat = subEnt->getMaterial();
					if (!mat.isNull())
					{
						...
						if (tech && tech->getNumPasses() > 0)
						{
							...
							if (pass)
							{
								pass->setDiffuse(bodyMark1.r, bodyMark1.g, bodyMark1.b, 1.0f);
							}
						}
					}
				}
	...
	// TODO: Реализовать раздельные цвета для body/head/tail маркировок
}
...
void MagixUnit::createNameTag(const String &name, MovableTextOverlayAttributes *attrs)
{
	...
	// Пробуем задать позицию через параметры конструктора (последние 3 параметра)
	mNameText = new MovableTextOverlay("NameTag" + StringConverter::toString(unitID), name, mHeadEnt, attrs, 0, 0.3, 0);
...
void MagixUnit::createUserTag(const String &name, MovableTextOverlayAttributes *attrs)
{
	...
	// Аналогично для UserTag
	mUserText = new MovableTextOverlay("UserTag" + StringConverter::toString(unitID), name, mHeadEnt, attrs, 0, 0.15, 16);
...
void MagixUnit::equip(SceneManager *sceneMgr, MagixExternalDefinitions *mDef, const String &meshName, const Vector3 &offset, const short &slot, bool altAnim)
{
	if (unitID == 0)
	{
		printf("EQUIP ERROR: unitID is 0\n");
		return;
	}
	if (slot < 0 || slot >= MAX_EQUIP)
	{
		printf("EQUIP ERROR: Invalid slot %d\n", slot);
		return;
	}
	if (mEquipEnt[slot])
	{
		printf("EQUIP ERROR: Slot %d already occupied\n", slot);
		return;
	}

	printf("EQUIP: Creating entity for slot %d, mesh: %s\n", slot, meshName.c_str());

	try
	{
		// Создаем ентити предмета
		String entityName = "Equip" + StringConverter::toString(slot) + "_" + StringConverter::toString(unitID);
		mEquipEnt[slot] = sceneMgr->createEntity(entityName, meshName + ".mesh");
		printf("EQUIP: Entity created: %s\n", entityName.c_str());

		// Сохраняем ентити в массиве
		setEquipEnt(slot, mEquipEnt[slot]);

		// Включаем анимацию если есть
		String animName = (altAnim ? "Equip2" : "Equip");
		if (mEquipEnt[slot]->getSkeleton() && mEquipEnt[slot]->getAnimationState(animName))
		{
			mEquipEnt[slot]->getAnimationState(animName)->setEnabled(true);
			printf("EQUIP: Animation %s enabled\n", animName.c_str());
		}

		// Получаем кость для прикрепления
		const String tBone = mDef->getItemBone(meshName);
		printf("EQUIP: Attaching to bone: %s\n", tBone.c_str());

		// Прикрепляем к кости скелета
		bool attached = false;
		if (mHeadEnt && StringUtil::startsWith(tBone, "Head"))
		{
			if (mHeadEnt->getSkeleton() && mHeadEnt->getSkeleton()->hasBone(tBone))
			{
				mHeadEnt->attachObjectToBone(tBone, mEquipEnt[slot], Quaternion::IDENTITY, offset);
				attached = true;
				printf("EQUIP: Attached to head bone: %s\n", tBone.c_str());
			}
		}
		else if (mTailEnt && StringUtil::startsWith(tBone, "Tail"))
		{
			if (mTailEnt->getSkeleton() && mTailEnt->getSkeleton()->hasBone(tBone))
			{
				mTailEnt->attachObjectToBone(tBone, mEquipEnt[slot], Quaternion::IDENTITY, offset);
				attached = true;
				printf("EQUIP: Attached to tail bone: %s\n", tBone.c_str());
			}
		}
		else if (mBodyEnt)
		{
			if (mBodyEnt->getSkeleton() && mBodyEnt->getSkeleton()->hasBone(tBone))
			{
				mBodyEnt->attachObjectToBone(tBone, mEquipEnt[slot], Quaternion::IDENTITY, offset);
				attached = true;
				printf("EQUIP: Attached to body bone: %s\n", tBone.c_str());
			}
		}

		if (!attached)
		{
			printf("EQUIP WARNING: Could not attach to any bone. Using body as fallback.\n");
			if (mBodyEnt && mBodyEnt->getSkeleton())
			{
				// Попробуем прикрепить к корневой кости тела
				mBodyEnt->attachObjectToBone("Root", mEquipEnt[slot], Quaternion::IDENTITY, offset);
				printf("EQUIP: Attached to fallback bone: Root\n");
			}
		}

		// Устанавливаем видимость
		mEquipEnt[slot]->setVisible(true);
		printf("EQUIP: Entity set to visible\n");

		// Устанавливаем маску запросов
		mEquipEnt[slot]->setQueryFlags(UNIT_MASK);

		printf("EQUIP SUCCESS: Item equipped in slot %d\n", slot);
	}
	catch (Ogre::Exception& e)
	{
		printf("EQUIP EXCEPTION: %s\n", e.getFullDescription().c_str());
		// Очищаем указатель если создание не удалось
		setEquipEnt(slot, 0);
	}
}
...
void MagixUnit::updateLipSync(const FrameEvent &evt)
{
	if (lipSyncAnim != "")
	{
		// Unweigh emote (existing code remains the same)
		if (mHeadEnt->getAnimationState("FaceNormal")->getEnabled() && mHeadEnt->getAnimationState("FaceNormal")->getWeight()>0)
			mHeadEnt->getAnimationState("FaceNormal")->setWeight(mHeadEnt->getAnimationState("FaceNormal")->getWeight() - evt.timeSinceLastFrame*lipSyncSpeed * 6);
		if (mHeadEnt->getAnimationState("FaceSmile")->getEnabled() && mHeadEnt->getAnimationState("FaceSmile")->getWeight()>0)
			mHeadEnt->getAnimationState("FaceSmile")->setWeight(mHeadEnt->getAnimationState("FaceSmile")->getWeight() - evt.timeSinceLastFrame*lipSyncSpeed * 6);
		if (mHeadEnt->getAnimationState("FaceOpenJaw")->getEnabled() && mHeadEnt->getAnimationState("FaceOpenJaw")->getWeight()>0)
			mHeadEnt->getAnimationState("FaceOpenJaw")->setWeight(mHeadEnt->getAnimationState("FaceOpenJaw")->getWeight() - evt.timeSinceLastFrame*lipSyncSpeed * 6);

		if (prevLipSyncAnim != "" && prevLipSyncAnim != lipSyncAnim)
			mHeadEnt->getAnimationState(prevLipSyncAnim)->addTime(-evt.timeSinceLastFrame*lipSyncSpeed * 4);
	...
	if (lipSyncCounter < 0)
	{
		...
		if (mHeadEnt->getAnimationState("FaceNormal")->getWeight() == 1 &&
			...
			(prevLipSyncAnim == "" || mHeadEnt->getAnimationState(prevLipSyncAnim)->getTimePosition() == 0))
		{
			if (prevLipSyncAnim != "")
	...
	// Use UTFString for proper Unicode handling
	UTFString utfLipSync(lipSync);
	if (utfLipSync.size() <= 0)
	{
		if (lipSyncAnim != "")
		{
	...
	while (utfLipSync.size() > 0 && utfLipSync[0] == '(')
	{
		size_t endPos = utfLipSync.find(')');
		if (endPos == UTFString::npos)
		{
			stopLipSync();
			return;
		}

		UTFString speedStr = utfLipSync.substr(1, endPos - 1);
		utfLipSync = utfLipSync.substr(endPos + 1);

		lipSyncSpeed = StringConverter::parseReal(speedStr.asUTF8());
		if (lipSyncSpeed < 0) lipSyncSpeed *= -1;
		if (lipSyncSpeed > 5) lipSyncSpeed = 5;

		if (utfLipSync.empty())
		{
			lipSync = utfLipSync.asUTF8();
			return;
		}
	}
	...
	// Get the first character as UTF-8 string and convert to wide char for comparison
	UTFString firstCharStr = utfLipSync.substr(0, 1);
	std::string firstCharUTF8 = firstCharStr.asUTF8();
	std::wstring firstCharWide = firstCharStr.asWStr();

	utfLipSync = utfLipSync.substr(1);
	lipSync = utfLipSync.asUTF8();

	// Map characters to mouth animations
	lipSyncAnim = "";

	if (firstCharWide.empty())
	{
		lipSyncCounter = 0.1;
		lipSyncAnim = "";
	}
	...
	{
		wchar_t nextChar = firstCharWide[0];

		// Russian vowels for wide open mouth
		if (nextChar == L'а' || nextChar == L'я' || nextChar == L'ё' ||
			nextChar == L'А' || nextChar == L'Я' || nextChar == L'Ё')
		{
			lipSyncAnim = "MouthA";
		}
		// Russian vowels for medium open mouth  
		else if (nextChar == L'ы' || nextChar == L'е' || nextChar == L'э' ||
			nextChar == L'и' || nextChar == L'й' ||
			nextChar == L'Ы' || nextChar == L'Е' || nextChar == L'Э' ||
			nextChar == L'И' || nextChar == L'Й')
		{
			lipSyncAnim = "MouthE";
		}
		// Russian vowels for rounded mouth
		else if (nextChar == L'о' || nextChar == L'у' || nextChar == L'ю' ||
			nextChar == L'О' || nextChar == L'У' || nextChar == L'Ю')
		{
			lipSyncAnim = "MouthO";
		}
		// Russian lip consonants
		else if (nextChar == L'м' || nextChar == L'б' || nextChar == L'п' ||
			nextChar == L'М' || nextChar == L'Б' || nextChar == L'П')
		{
			lipSyncAnim = "MouthM";
		}
		// Russian teeth and lip consonants
		else if (nextChar == L'ф' || nextChar == L'в' || nextChar == L'ж' ||
			nextChar == L'ш' || nextChar == L'щ' || nextChar == L'ч' ||
			nextChar == L'ц' ||
			nextChar == L'Ф' || nextChar == L'В' || nextChar == L'Ж' ||
			nextChar == L'Ш' || nextChar == L'Щ' || nextChar == L'Ч' ||
			nextChar == L'Ц')
		{
			lipSyncAnim = "MouthF";
		}
		// Russian tongue and dental consonants
		else if (nextChar == L'р' || nextChar == L'л' || nextChar == L'т' ||
			nextChar == L'д' || nextChar == L'к' || nextChar == L'г' ||
			nextChar == L'х' || nextChar == L'с' || nextChar == L'з' ||
			nextChar == L'н' ||
			nextChar == L'Р' || nextChar == L'Л' || nextChar == L'Т' ||
			nextChar == L'Д' || nextChar == L'К' || nextChar == L'Г' ||
			nextChar == L'Х' || nextChar == L'С' || nextChar == L'З' ||
			nextChar == L'Н')
		{
		...
		// English vowels and consonants (fallback)
		else if (nextChar == 'a' || nextChar == 'o' || nextChar == 'A' || nextChar == 'O')
		{
			lipSyncAnim = "MouthA";
		}
		...
		else if (nextChar == 'm' || nextChar == 'b' || nextChar == 'p' ||
			nextChar == 'M' || nextChar == 'B' || nextChar == 'P')
		{
			lipSyncAnim = "MouthM";
		}
		else if (nextChar == 'f' || nextChar == 'v' || nextChar == 'w' ||
			nextChar == 'F' || nextChar == 'V' || nextChar == 'W')
		{
			lipSyncAnim = "MouthF";
		}
		else if (nextChar == 'l' || nextChar == 'r' || nextChar == 't' ||
			nextChar == 'd' || nextChar == 'n' || nextChar == 's' ||
			nextChar == 'z' || nextChar == 'j' ||
			nextChar == 'L' || nextChar == 'R' || nextChar == 'T' ||
			nextChar == 'D' || nextChar == 'N' || nextChar == 'S' ||
			nextChar == 'Z' || nextChar == 'J')
		{
			lipSyncAnim = "MouthL";
	...
	// If same animation as previous, add small delay
	if (lipSyncAnim == prevLipSyncAnim && lipSyncAnim != "")
	{
		lipSyncCounter = 0.2;
		return;
	...
	if (lipSyncAnim != "")
	{
		if (!mHeadEnt->getAnimationState(lipSyncAnim)->getEnabled())
		{
...
Entity* MagixUnit::getEquipEnt(unsigned short slot)
{
	if (slot >= MAX_EQUIP) return 0;
	return mEquipEnt[slot];
}

void MagixUnit::setEquipEnt(unsigned short slot, Entity* ent)
{
	if (slot >= MAX_EQUIP) return;
	mEquipEnt[slot] = ent;
}
