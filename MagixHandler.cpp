...
void MagixHandler::updateCommand()
{
	...
	else if (tCommand == COMMAND_WEATHER)
	{
		if (!mDef->isAdmin && !mDef->isMod)
		{
			mGui->getChatManager()->message("You don't have permission to change weather.");
			return;
		}

		if (tParam == "")
		{
			mGui->getChatManager()->message("Usage: /weather <type> or /weather cycle <cycle_name> or /weather clear");
			mGui->getChatManager()->message("Available weather types: Rain, Snow, Starfall, Fireworks, Thunderstorm");
			return;
		}
		...
		if (tParam == "clear")
		{
			...
			mSkyManager->setWeatherCycle("");
			mGui->getChatManager()->message("Weather cleared");

			// Îòïðàâëÿåì íà ñåðâåð äëÿ âñåõ
			mNetworkManager->sendWeather("clear");
		}
		else if (tParam.find("cycle ") == 0)
		{
			String cycleName = tParam.substr(6);
			// Ëîêàëüíî
			mSkyManager->setWeatherCycle(cycleName);
			mGui->getChatManager()->message("Weather cycle set to: " + cycleName);

			// Îòïðàâëÿåì íà ñåðâåð äëÿ âñåõ
			mNetworkManager->sendWeather("cycle " + cycleName);
		}
		else
		{
			String normalizedType = tParam;
			...
			mGui->getChatManager()->message("Setting weather to: " + normalizedType);
			...
			if (normalizedType == "Rain") {
				...
				mGui->getChatManager()->message("Unknown weather type: " + normalizedType);
				return;
			}

			// ÓÄÀËßÅÌ ÑÒÀÐÓÞ ÑÈÑÒÅÌÓ ÏÅÐÅÄ ÑÎÇÄÀÍÈÅÌ ÍÎÂÎÉ
			if (mSceneMgr->hasParticleSystem("WeatherSystem"))
			{
				mSceneMgr->destroyParticleSystem("WeatherSystem");
			}

			// ÑÎÇÄÀÅÌ ÍÀÏÐßÌÓÞ
			ParticleSystem* weatherSystem = mSceneMgr->createParticleSystem("WeatherSystem", particleSystem);
			if (weatherSystem)
			{
				weatherSystem->setEmitting(true);
				weatherSystem->setVisible(true);

				SceneNode* weatherNode = mSceneMgr->getSceneNode("WeatherOffsetNode");
				if (weatherNode)
				{
					...
					mGui->getChatManager()->message(normalizedType + " weather created successfully!");
				}
			}

			// Îòïðàâëÿåì íà ñåðâåð äëÿ âñåõ
			mNetworkManager->sendWeather(normalizedType);
		}
	}
}
