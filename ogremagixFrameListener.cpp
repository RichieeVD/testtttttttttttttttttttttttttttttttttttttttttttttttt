	...
	mMagixHandler(magixHandler)
{
	...
	// ДОБАВЛЕНО: инициализация chatManager с клавиатурой
	// mMagixHandler->getMagixGUI()->getChatManager()->initializeInput(mKeyboard);
}
...
bool ogremagixFrameListener::keyPressed(const OIS::KeyEvent &e)
{
	...
	// ПРОСТАЯ ОБРАБОТКА Ctrl+V
	if (mMagixHandler->getInputMode() == INPUT_TEXT && e.key == KC_V)
	{
		if (mKeyboard->isKeyDown(KC_LCONTROL) || mKeyboard->isKeyDown(KC_RCONTROL))
		{
			// Вызываем вставку напрямую через GUI
			mMagixHandler->getMagixGUI()->pasteToChat();
			return true;
		}
	}
	...
	{
		// Если нажата V - вызываем вставку из буфера
		if (e.key == KC_V)
		{
			MagixChatManager *chatManager = mMagixHandler->getMagixGUI()->getChatManager();
			if (chatManager)
			{
				chatManager->pasteFromClipboard();
				return true;
			}
		}

		// Обычная обработка других клавиш
		if (mMagixHandler->getMagixGUI()->getChatManager()->handleKeyPress(e.key, false))
		{
			return true;
		}
	}
		...
		switch (e.key)
	{
		...
		case KC_E:
			...
			{
				// ДОБАВЛЕНО: подбор предмета по клавише E
...
bool ogremagixFrameListener::keyReleased(const OIS::KeyEvent &e)
{
	...
	if (mMagixHandler->getInputMode() == INPUT_TEXT)
	{
		mMagixHandler->getMagixGUI()->getChatManager()->handleKeyRelease(e.key);
	}
