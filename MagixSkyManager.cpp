...
void MagixSkyManager::setWeather(const String &type)
{
	...
	LogManager::getSingleton().logMessage("DEBUG: Setting weather - Type: " + type +
		", Particle: " + tTypeName +
		", Offset: " + StringConverter::toString(tOffset) +
		", Sound: " + tSoundName);
	...
	if (tTypeName == "")
	{
		LogManager::getSingleton().logMessage("DEBUG: Weather particle system not found for: " + type);
		return;
	}
	...
	try
	{
		...
		if (!mWeatherSystem)
		{
			LogManager::getSingleton().logMessage("DEBUG: Failed to create particle system: " + tTypeName);
			return;
		}

		// КРИТИЧЕСКИ ВАЖНО: Принудительно запускаем систему
		mWeatherSystem->setEmitting(true);
		mWeatherSystem->setSpeedFactor(1.0f); // Убедимся, что скорость нормальная

		mWeatherSystem->setQueryFlags(SKY_MASK);

		SceneNode *tWeatherOffsetNode = mSceneMgr->getSceneNode("WeatherOffsetNode");
		if (!tWeatherOffsetNode)
		{
			LogManager::getSingleton().logMessage("DEBUG: Creating new WeatherOffsetNode");
			tWeatherOffsetNode = mWeatherNode->createChildSceneNode("WeatherOffsetNode");
		}

		tWeatherOffsetNode->attachObject(mWeatherSystem);
		tWeatherOffsetNode->setPosition(tOffset);

		// Принудительно включаем ВСЁ
		mWeatherSystem->setVisible(true);
		mWeatherNode->setVisible(true);
		tWeatherOffsetNode->setVisible(true);

		// Немедленно обновляем позицию
		updateSky();

		// Дополнительная отладка
		LogManager::getSingleton().logMessage("DEBUG: Weather system created - Emitting: " +
			StringConverter::toString(mWeatherSystem->getEmitting()) +
			", NumEmitters: " + StringConverter::toString(mWeatherSystem->getNumEmitters()));

		if (mWeatherSystem->getNumEmitters() > 0)
		{
			ParticleEmitter* emitter = mWeatherSystem->getEmitter(0);
			LogManager::getSingleton().logMessage("DEBUG: Emitter enabled: " +
				StringConverter::toString(emitter->getEnabled()) +
				", EmissionRate: " + StringConverter::toString(emitter->getEmissionRate()));
		}

	}
	catch (Exception &e)
	{
		LogManager::getSingleton().logMessage("DEBUG: Error creating weather system: " + e.getFullDescription());
		return;
	}
	...
	if (hasWeatherSound)
	{
		...
		LogManager::getSingleton().logMessage("DEBUG: Weather sound started: " + tSoundName);
	}
}
void MagixSkyManager::destroyWeather()
{
	...
	if (tWeatherOffsetNode)
	{
		...
		SceneNode::ObjectIterator it = tWeatherOffsetNode->getAttachedObjectIterator();
		while (it.hasMoreElements())
		{
			MovableObject* obj = it.getNext();
			if (obj->getMovableType() == "ParticleSystem")
			{
				tWeatherOffsetNode->detachObject(obj);
	...
	if (mWeatherSystem)
	{
		...
		if (tWeatherOffsetNode && mWeatherSystem->isAttached())
		{
...
void MagixSkyManager::setWeatherSystemDirect(ParticleSystem* system, const String& type)
{

	mWeatherSystem = system;
	weatherType = type;
	hasWeatherSound = false;
}