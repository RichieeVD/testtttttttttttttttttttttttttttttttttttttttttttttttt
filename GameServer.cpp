	...
	while (alive)
	{
		...
		*/
		while (p && tCont)
		{
			...
			case ID_WEATHER:
			{
				printf("=== WEATHER PACKET RECEIVED ===\n");
				...
				tReceiveBit.Read(tMessage);
				tReceiveBit.Read(tSender); // ◊ËÚ‡ÂÏ ÚÓÍÂÌ ÓÚÔ‡‚ËÚÂÎˇ
				StringCompressor::Instance()->DecodeString(tWeatherType, 32, &tReceiveBit);

				printf("Weather type: %s from sender: %i\n", tWeatherType, tSender);

				if (tSender > 0 && tSender <= MAX_CLIENTS)
				{
					string tMapName = clientMap[tSender - 1];
					unsigned char tDimension = clientDimension[tSender - 1];

					printf("Map: %s, Dimension: %i\n", tMapName.c_str(), tDimension);

					// –Â·Ó‰Í‡ÒÚËÏ ÔÓ„Ó‰Û ‚ÒÂÏ ‚ ˝ÚÓÈ Í‡ÚÂ/ËÁÏÂÂÌËË
					RakNet::BitStream tBitStream;
					tBitStream.Write(MessageID(ID_WEATHER));
					tBitStream.Write(tSender); // ŒÚÔ‡‚ÎˇÂÏ ÚÓÍÂÌ ÓÚÔ‡‚ËÚÂÎˇ
					StringCompressor::Instance()->EncodeString(tWeatherType, 32, &tBitStream);

					broadcastInMap(tMapName, tDimension, server, &tBitStream, HIGH_PRIORITY, RELIABLE, 0, UNASSIGNED_SYSTEM_ADDRESS);
					printf("Broadcasted weather '%s' to map: %s\n", tWeatherType, tMapName.c_str());
				}
				else
				{
					printf("ERROR: Invalid sender token: %i\n", tSender);
				}
			}
			break;
			...
			case ID_ITEMDROP:
			{
				...
				// ===== ¬À¿ƒ: —Œ’–¿Õﬂ≈Ã œ–≈ƒÃ≈“ (Ã¿ — 5 ÿ“” ) =====
				SavedItem newItem;
				newItem.ID = tID;
				strcpy(newItem.mesh, tMesh);
				newItem.x = tX;
				newItem.z = tZ;
				newItem.mapName = tMapName;
				newItem.dimension = tDimension;

				// À»Ã»“ 100 œ–≈ƒÃ≈“Œ¬ + ”ƒ¿À≈Õ»≈ ”  À»≈Õ“Œ¬
				if (savedItems.size() >= 100) {
					// ”‰‡ÎˇÂÏ —¿Ã€… —“¿–€… ÔÂ‰ÏÂÚ Ë ÒÓÓ·˘‡ÂÏ ÍÎËÂÌÚ‡Ï
					SavedItem oldItem = savedItems[0];

					// –‡ÒÒ˚Î‡ÂÏ Û‰‡ÎÂÌËÂ ÒÚ‡Ó„Ó ÔÂ‰ÏÂÚ‡ ‚ÒÂÏ Ë„ÓÍ‡Ï Í‡Ú˚
					RakNet::BitStream removeBitStream;
					removeBitStream.Write(MessageID(ID_ITEMPICKUP));
					removeBitStream.Write(oldItem.ID);
					removeBitStream.Write(false); // false = ÌÂ ÔÓ‰·Ó, ‡ Û‰‡ÎÂÌËÂ

					broadcastInMap(oldItem.mapName, oldItem.dimension, server, &removeBitStream, MEDIUM_PRIORITY, RELIABLE, 2, UNASSIGNED_SYSTEM_ADDRESS);
					printf("”‰‡ÎËÎ ÒÚ‡˚È ÔÂ‰ÏÂÚ ID %i! —ÓÓ·˘ËÎ ÍÎËÂÌÚ‡Ï. ¬ÒÂ„Ó: %i\n", oldItem.ID, savedItems.size());

					savedItems.erase(savedItems.begin());
				}

				savedItems.push_back(newItem);
				printf("—Óı‡ÌÂÌ ÔÂ‰ÏÂÚ %s (ID:%i) Ì‡ Í‡ÚÂ %s. ¬ÒÂ„Ó: %i/5\n", tMesh, tID, tMapName.c_str(), savedItems.size());
				// =====  ŒÕ≈÷ ¬À¿ƒ =====
			...
			case ID_ITEMPICKUP:
			{
				...
				// ===== ¬À¿ƒ: ”ƒ¿Àﬂ≈Ã œŒƒŒ¡–¿ÕÕ€… œ–≈ƒÃ≈“ »« —Œ’–¿Õ≈ÕÕ€’ =====
				for (int i = 0; i < savedItems.size(); i++) {
					if (savedItems[i].ID == tID && savedItems[i].mapName == tMapName) {
						savedItems.erase(savedItems.begin() + i);
						printf("”‰‡ÎÂÌ ÔÓ‰Ó·‡ÌÌ˚È ÔÂ‰ÏÂÚ ID: %i. ŒÒÚ‡ÎÓÒ¸: %i\n", tID, savedItems.size());
						break;
					}
				}
				// =====  ŒÕ≈÷ ¬À¿ƒ =====

				// ... ÓÒÚ‡Î¸ÌÓÈ ÒÛ˘ÂÒÚ‚Û˛˘ËÈ ÍÓ‰ ...
	{
		//Send Item data
		RakNet::BitStream tBitStream;
...
void ServerManager::sendNewPlayerProcedure(const OwnerToken &playerToken, const string &mapName, const unsigned char &dimension)
{
	...
	// ===== ¬À¿ƒ: Œ“œ–¿¬Àﬂ≈Ã ¬—≈ œ–≈ƒÃ≈“€  ¿–“€ ÕŒ¬ŒÃ” »√–Œ ” =====
	for (int i = 0; i < savedItems.size(); i++) {
		SavedItem item = savedItems[i];
		if (item.mapName == mapName && item.dimension == dimension) {
			RakNet::BitStream tBitStream;
			tBitStream.Write(MessageID(ID_ITEMDROP));
			tBitStream.Write(item.ID);
			StringCompressor::Instance()->EncodeString(item.mesh, 16, &tBitStream);
			tBitStream.Write(item.x);
			tBitStream.Write(item.z);

			server->Send(&tBitStream, MEDIUM_PRIORITY, RELIABLE, 2, clientAdd[playerToken - 1], false);
			printf("ŒÚÔ‡‚ÎÂÌ ÔÂ‰ÏÂÚ %s Ë„ÓÍÛ %i\n", item.mesh, playerToken);
		}
	}
	// =====  ŒÕ≈÷ ¬À¿ƒ =====
	if (tHasOtherPlayers)
