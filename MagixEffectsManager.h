...
protected:
	...
	SceneNode *mItemEffectOwnerNode[MAX_ITEMEFFECTS];
	String itemEffectType[MAX_ITEMEFFECTS];
	Vector3 itemEffectOffset[MAX_ITEMEFFECTS];
	String itemEffectBone[MAX_ITEMEFFECTS];
	String itemEffectUniqueID[MAX_ITEMEFFECTS]; // NEW: Уникальные идентификаторы эффектов
...
public:
	MagixEffectsManager()
	{
		...
		for (int i = 0; i<MAX_ITEMEFFECTS; i++)
		{
			...
			mItemEffectOwnerNode[i] = 0;
			itemEffectType[i] = "";
			itemEffectOffset[i] = Vector3::ZERO;
			itemEffectBone[i] = "";
			itemEffectUniqueID[i] = ""; // NEW: Инициализация уникальных ID
		}
	}
	~MagixEffectsManager()
	{
	...
	void createItemParticle(SceneNode *node, const String &particleType = "item");

	void destroyItemParticle(unsigned short tID);

	void destroyItemParticleByObjectNode(SceneNode *node);

	// Обновленные методы для создания эффектов с уникальными идентификаторами
	void createItemEffect(SceneNode *ownerNode, const String &type, const String &bone, const Vector3 &offset, const Real &scale, const String &uniqueID = "");

	void destroyItemEffect(unsigned short tID);

	void destroyItemEffectByOwnerNode(SceneNode *node, const String &type = "");

	// NEW: Методы для работы с уникальными идентификаторами
	void destroyItemEffectByUniqueID(const String &uniqueID);
	bool hasItemEffectWithUniqueID(const String &uniqueID);
};