...
{
	// Удаляем старого Critter с таким ID
	deleteCritter(iID);

	// Создаем Entity из исходного меша
	...
	ent->setQueryFlags(CRITTER_MASK); // важно для выбора и Select Panel
	...
	tC->setIsCraftingStation(critter.isCraftingStation);

	// Эффекты для Draw Point
	...
	{
		// Удаляем старый Entity
		mSceneMgr->destroyEntity(tC->getEnt());

		// Создаем новый Entity для крафт-станции
		Entity* newEnt = mSceneMgr->createEntity("Critter" + StringConverter::toString(tC->getID()), "Egg.mesh");
		newEnt->setQueryFlags(CRITTER_MASK); // важно для Select Panel и кружка
		tC->getObjectNode()->attachObject(newEnt);
		tC->getObjectNode()->setScale(2, 2, 2);
		tC->setEnt(newEnt);

		// Добавляем эффект круга вызова как у DrawPoint
		mEffectsManager->createParticle(tC->getObjectNode(), "DrawPoint", -1);

		// Добавляем в список станций крафта в World
		if (mWorld)
			mWorld->addCraftingStation(tC->getObjectNode());
	}
	// === КОНЕЦ КРАФТ СИСТЕМЫ === //

	// Добавляем Critter в списки
