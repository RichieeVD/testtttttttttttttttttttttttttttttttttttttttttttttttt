...
void MagixCamera::initialize(SceneManager *sceneMgr, MagixPlayer *player, MagixCollisionManager *collMgr)
{
	...
	// ÓÇÅË ÄËß ÊÀÌÅÐÛ ÎÒÐÀÆÅÍÈÉ - ÎÒÄÅËÜÍÛÉ!
	mReflectCamNode = mCamPivotPitchNode->createChildSceneNode("ReflectCameraNode");

	mCamTrackNode = sceneMgr->getRootSceneNode()->createChildSceneNode("CamTrackNode");

	// ÏÐÈÊÐÅÏËßÅÌ ÊÀÌÅÐÛ Ê ÐÀÇÍÛÌ ÓÇËÀÌ
	mCamNode->attachObject(mCamera);
	mReflectCamNode->attachObject(mReflectCamera);
}
...
void MagixCamera::setCameraMode(const unsigned char &mode, bool firstPerson, bool forceUpdate)
{
	...
	case CAMERA_FREE:
		...
		if (mReflectCamera->isAttached())mReflectCamNode->detachObject(mReflectCamera);
...
void MagixCamera::doCameraTrack(bool loop, bool autoTrack, SceneNode *node)
{
	...
	// ÈÑÏÐÀÂËÅÍÎ: äëÿ êàìåðû îòðàæåíèé òîæå èñïîëüçóåì îòäåëüíûé óçåë
	if (camMode != CAMERA_FREE && mReflectCamera->isAttached())mReflectCamNode->detachObject(mReflectCamera);

	// Ñîçäàåì îòäåëüíûé óçåë äëÿ êàìåðû îòðàæåíèé â òðåêå
	SceneNode* reflectTrackNode = mCamTrackNode->createChildSceneNode("ReflectTrackNode");
	reflectTrackNode->attachObject(mReflectCamera);
...
void MagixCamera::stopCameraTrack()
{
	...
	// ÈÑÏÐÀÂËÅÍÎ: âîññòàíàâëèâàåì êàìåðó îòðàæåíèé (ïðîñòàÿ âåðñèÿ)
	// Èùåì äî÷åðíèé óçåë ñ êàìåðîé îòðàæåíèé è óäàëÿåì åãî
	SceneNode::ChildNodeIterator it = mCamTrackNode->getChildIterator();
	while (it.hasMoreElements())
	{
		SceneNode* child = static_cast<SceneNode*>(it.getNext());
		if (child->getName() == "ReflectTrackNode")
		{
			child->detachAllObjects();
			mCamTrackNode->removeAndDestroyChild(child->getName());
			break;
		}
	}

	if (!mReflectCamera->isAttached() && camMode != CAMERA_FREE)
	{
		mReflectCamNode->attachObject(mReflectCamera);
	}
}
...
{
	Vector3 camPos = (mCamera->isAttached() ? mCamera->getDerivedPosition() : mCamera->getPosition());
	isUnderwater = (mCollisionManager->getWaterBoxHit(camPos) != 0);

	// ÓÁÈÐÀÅÌ setFog - ýòîò ìåòîä íå ñóùåñòâóåò â Camera
	// Âìåñòî ýòîãî ìîæíî äîáàâèòü äðóãóþ ëîãèêó äëÿ ïîäâîäíûõ ýôôåêòîâ
}
