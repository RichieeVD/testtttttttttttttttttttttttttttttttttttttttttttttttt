...
void MagixGUI::initializeEmojis()
{
	if (mEmojisInitialized) return;
	loadEmojisFromConfig();

	mEmojiFontMap.clear();
	if (mEmojiFontMap.empty()) {
		// Более структурированная система смайликов
		std::vector<EmojiDefinition> emojiDefs = {
			{ ":)", "Webdings", "J", true, "Улыбка" },
			{ ":(", "Webdings", "L", true, "Грусть" },
			{ ":D", "Webdings", "K", true, "Смех" },
			{ ";)", "Webdings", ";", true, "Подмигивание" },
			{ ":P", "Webdings", "R", true, "Язык" },
			{ ":O", "Webdings", "N", true, "Удивление" },
			{ "<3", "Webdings", "Y", true, "Сердце" },
			{ ":sob:", "Webdings", "M", true, "Плач" },
			{ ":rofl:", "Webdings", "G", true, "Ржач" },
			{ ":fire:", "Webdings", "!", true, "Огонь" },
			{ ":heart:", "Webdings", "Y", true, "Красное сердце" },
			{ ":star:", "Webdings", "(", true, "Звезда" },
			{ ":like:", "Webdings", "6", true, "Лайк" }
		};

		for (const auto& def : emojiDefs) {
			mEmojiFontMap[def.sequence] = { def.fontName, def.character, def.useFont };
		}
	}
	// Сортируем по длине (от длинных к коротким) для правильной замены
	mEmojiReplaceOrder.clear();
	for (const auto& pair : mEmojiFontMap) {
		mEmojiReplaceOrder.push_back(pair.first);
	}
	std::sort(mEmojiReplaceOrder.begin(), mEmojiReplaceOrder.end(),
		[](const String& a, const String& b) {
		return a.length() > b.length();
	});

	mEmojisInitialized = true;
	LogManager::getSingleton().logMessage("Enhanced emoji system initialized with " +
		StringConverter::toString(mEmojiFontMap.size()) + " emojis");
}
...
String MagixGUI::replaceEmojisInText(const String& text)
{
	...
	for (const auto& emojiSeq : mEmojiReplaceOrder) {
		if (emojiSeq.length() <= 2) continue; // Короткие обработаем отдельно

		size_t pos = 0;
		while ((pos = result.find(emojiSeq, pos)) != String::npos) {
			// Проверяем границы слова для длинных смайликов
			if (isValidEmojiBoundary(result, pos, emojiSeq.length())) {
				const EmojiFontInfo& info = mEmojiFontMap[emojiSeq];
				result.replace(pos, emojiSeq.length(), info.character);
				pos += info.character.length();
			}
			else {
				pos += emojiSeq.length();
			}
	...
	// Затем заменяем короткие последовательности (типа :), :D)
	for (const auto& emojiSeq : mEmojiReplaceOrder) {
		if (emojiSeq.length() > 2) continue; // Длинные уже обработали
...
bool MagixGUI::isValidEmojiBoundary(const String& text, size_t pos, size_t length)
{
	// Для пустой строки или некорректной позиции
	if (text.empty() || pos >= text.length()) return false;

	// Проверяем символ перед смайликом
	if (pos > 0) {
		unsigned char prevChar = text[pos - 1];
		// Более безопасная проверка границ
		if (isalnum(prevChar) || prevChar == '_' || prevChar > 127) {
			return false;
		}
	}

	// Проверяем символ после смайлика
	if (pos + length < text.length()) {
		unsigned char nextChar = text[pos + length];
		if (isalnum(nextChar) || nextChar == '_' || nextChar > 127) {
			return false;
		}
	}
...
void MagixGUI::loadEmojisFromConfig()
{
	ConfigFile cfg;
	try {
		cfg.load("emojis.cfg");
		...
		while (it.hasMoreElements()) {
			String sequence = it.peekNextKey();
			String fontInfo = it.getNext();

			vector<String>::type parts = StringUtil::split(fontInfo, ",");
			if (parts.size() >= 2) {
				String fontName = parts[0];
				String character = parts[1];
				bool useFont = parts.size() > 2 ? StringConverter::parseBool(parts[2]) : true;

				mEmojiFontMap[sequence] = { fontName, character, useFont };
			}
		}
	}
	catch (...) {
		LogManager::getSingleton().logMessage("Failed to load emoji config, using defaults");
		initializeEmojis(); // Fallback to defaults
	}
}
...
{
	LogManager::getSingleton().logMessage("=== Available Emojis ===");
	for (const auto& pair : mEmojiFontMap) {
		LogManager::getSingleton().logMessage(pair.first + " -> '" +
			pair.second.character + "' (" + pair.second.fontName + ")");
	}
}

bool MagixGUI::isEmojiSequence(const String& sequence)
{
	return mEmojiFontMap.find(sequence) != mEmojiFontMap.end();
}


void MagixGUI::addEmoji(const String& sequence, const String& fontName, const String& character)
{
	mEmojiFontMap[sequence] = { fontName, character, true };

	// Обновляем порядок замены
	mEmojiReplaceOrder.push_back(sequence);
	std::sort(mEmojiReplaceOrder.begin(), mEmojiReplaceOrder.end(),
		[](const String& a, const String& b) {
		return a.length() > b.length();
	});
...
String MagixGUI::processTextWithMixedFonts(const String& text)
{
	if (!mEmojisInitialized) return text;
	...
	for (const auto& emojiSeq : mEmojiReplaceOrder) {
		size_t searchPos = 0;
		while ((searchPos = text.find(emojiSeq, searchPos)) != String::npos) {
			if (isValidEmojiBoundary(text, searchPos, emojiSeq.length())) {
				matches.push_back({ searchPos, emojiSeq });
			}
			searchPos += emojiSeq.length();
		}
	}

	// Сортируем matches по позиции
	std::sort(matches.begin(), matches.end(),
		[](const EmojiMatch& a, const EmojiMatch& b) {
		return a.position < b.position;
	});

	// Строим результат с правильным порядком
	lastPos = 0;
	for (const auto& match : matches) {
		// Добавляем текст до смайлика
		if (match.position > lastPos) {
			result += text.substr(lastPos, match.position - lastPos);
		}

		// Добавляем смайлик
		const EmojiFontInfo& info = mEmojiFontMap[match.sequence];
		result += info.character;

		lastPos = match.position + match.sequence.length();
	}
...
void MagixGUI::cleanupEmojis()
{
	mEmojiFontMap.clear();
	mEmojisInitialized = false;
}

String MagixGUI::processTextWithEmojis(const String& text)
{
	return replaceEmojisInText(text);
}

// ==================== ОБНОВЛЕННЫЙ МЕТОД updateChatBox ====================

void MagixGUI::updateChatBox(const FrameEvent &evt)
{
	if (!mBox[GUI_TEXTOUTPUT]->isVisible() || !mBoxText[GUI_TEXTOUTPUT]->isVisible()) {
		return;
	}

	if (mChatManager->popHasNewLine(mChatManager->getChannel()) && scrollerHeldID != SCROLLER_TEXTOUTPUT)
		setScroll(SCROLLER_TEXTOUTPUT, 1);
	...
	// Получаем текст чата
	String chatText = mChatManager->getChatBlock(Math::IFloor(tLines),
		mBoxText[GUI_TEXTOUTPUT]->getWidth() - 0.09, tCharHeight,
		(1 - getScroll(SCROLLER_TEXTOUTPUT)));

	// Обрабатываем смайлики в тексте
	String processedText = processTextWithEmojis(chatText);

	// Устанавливаем обработанный текст
	mBoxText[GUI_TEXTOUTPUT]->setCaption(processedText);
	mBoxText[GUI_TEXTOUTPUT]->setTop((tLines - Math::IFloor(tLines)) * tCharHeight + 0.01);
}

// ==================== ОБНОВЛЕННЫЙ МЕТОД showChatBox ====================

void MagixGUI::showChatBox(bool flag, bool showInput)
{
	if (!flag) {
		if (mEmojiTextArea) mEmojiTextArea->hide();
		...
		if (mEmojiTextArea) mEmojiTextArea->show();
...
void MagixGUI::update(const FrameEvent &evt)
{
	...
	// Проверка нажатия E для подбора предметов
	...
	updateChatBox(evt);
	updateEmojis(evt.timeSinceLastFrame);
...
void MagixGUI::updateStashBox()
{
	...
	{
		const String itemName = tStash[i];
		const unsigned short count = mItemManager->getStashCountAt(i);

		String displayName = mDef->getItemName(itemName);
		if (count > 1)
		{
			displayName += " x" + StringConverter::toString(count);
		}

		tList.push_back(displayName);
	}

	updateBoxScrollerCaption(GUI_STASHBOX, SCROLLER_STASHBOX, tList);

	const String counterText = "-Stash- (" + StringConverter::toString(mItemManager->getTotalItemCount()) + "/30)";

	OverlayElement* stashMoverText = OverlayManager::getSingleton().getOverlayElement("GUI/StashMoverText");
	if (stashMoverText)
	{
		stashMoverText->setCaption(counterText);
	}
}
...
// МЕТОД ДЛЯ ОБРАБОТКИ СМАЙЛИКОВ
UTFString MagixGUI::processEmojis(const UTFString &text)
{
	// Конвертируем UTFString в String для обработки
	String regularText = text;

	// Используем ту же функцию замены
	String processed = replaceEmojisInText(regularText);
...
#pragma warning(pop)
		...
		if (mHoverWindow == mBox[GUI_STASHBOX])    //stashbox options
		{
			...
			// ПОКАЗЫВАЕМ ДВЕ КНОПКИ: Equip и Drop
			mButtonText[BUTTON_LISTSELECT1]->setCaption("Equip");
			mButtonText[BUTTON_LISTSELECT2]->setCaption("Drop");
			mButton[BUTTON_LISTSELECT2]->show();  // ← ТЕПЕРЬ ПОКАЗЫВАЕМ
			mButton[BUTTON_LISTSELECT3]->hide();
			mBox[GUI_LISTSELECTBOX]->setDimensions(0.1, 0.08);  // ← УВЕЛИЧИЛИ ВЫСОТУ ДЛЯ ДВУХ КНОПОК
			mBox[GUI_LISTSELECTBOX]->setPosition((Real)(cursorX + 0.02), (Real)(cursorY + 0.02));
...
void MagixGUI::mouseLeftUp()
{
	...
	if (mHoverButton)
		if (isCursorOverButton(mHoverButton))
		{
			...
			if (mHoverButton == mButton[BUTTON_STASHDROP])    // Drop from stash
			{
				dropSelectedStashItem();
				return;
			}
			...
			if (mHoverButton == mButton[BUTTON_LISTSELECT1] || mHoverButton == mButton[BUTTON_LISTSELECT2] || mHoverButton == mButton[BUTTON_LISTSELECT3]) //listselect buttons
			{
				...
				{
					if (mListSelectTarget == mBox[GUI_ITEMBOX])
					{
						mChatManager->message("Dropped " + listButtonData + ".");
						unequipItem(listButtonLine);  // doDrop = true по умолчанию
					}
					else if (mListSelectTarget == mBox[GUI_STASHBOX])
					{
						dropSelectedStashItem();
				...
				else if (tFunction == "Equip")
				{
					if (!mGameStateManager->isCampaign())
					{
						...
						if (checkItemEquippable(tItem, false, false))
						{
							...
							// ОБНОВЛЯЕМ СЧЕТЧИК ПОСЛЕ ИЗВЛЕЧЕНИЯ ИЗ СТАША
							updateStashBox();
						}
					}
				}
...
void MagixGUI::pasteToChat()
{
	if (!mInputManager) return;
	if (mInputManager->getInputMode() != INPUT_TEXT) return;
	if (!mBoxText[GUI_TEXTINPUT]) return;
	if (mInputManager->getInputBox() != mBoxText[GUI_TEXTINPUT]) return;
		...
		if (hData)
		{
			...
			if (text)
			{
				try
				{
					...
					String currentText = mInputManager->getInputText();

					// Добавляем текст из буфера
					String newText = currentText + text;

					// Устанавливаем новый текст через InputManager
					if (newText.length() < 256)
					{
						mInputManager->setInputText(newText, true);
					}
				}
				catch (...)
				{
...
// ОБНОВЛЕННЫЙ МЕТОД toggleInputMode С ПОДДЕРЖКОЙ UTFString
void MagixGUI::toggleInputMode(bool isEnter, OverlayElement *inputBox, UTFString defaultCaption, unsigned short charLimit, bool clearAtReturn, UTFString startText, bool allowTempStorage, bool allowNewLine, bool hideText)
{
	...
	if (mInputManager->toggleInputMode())
	{
		if (mInputManager->getInputBox() == mBoxText[GUI_TEXTINPUT])
		{
			...
			// Обрабатываем смайлики в введенном тексте
			tCaption = processEmojis(tCaption);
...
bool MagixGUI::selectCurrentObject(SceneNode* object, const unsigned int& flags, bool isRightClick)
{
	...
	if (flags == CRITTER_MASK)
	{
		...
		// Определяем тип Critter
		bool isCraftStation = tCritter->getIsCraftingStation();
		bool isDrawPoint = tCritter->getIsDrawPoint();
		bool isPet = tCritter->getIsPet() && tCritter->imTheOwner();

		// === Показываем Select Panel всегда ===
		showSelectPanel(true,
			isCraftStation ? "Crafting Station" :
			isDrawPoint ? "Draw Point" :
			tCritter->getCritterType());

		// === Настраиваем кнопки ===
		if (isCraftStation)
		{
			mButtonText[BUTTON_SELECTBOX1]->setCaption("Craft");
			mButtonText[BUTTON_SELECTBOX2]->setCaption("");
			mButtonText[BUTTON_SELECTBOX3]->setCaption("");
			mButtonText[BUTTON_SELECTBOX4]->setCaption("");
		}
		...
		{
			mButtonText[BUTTON_SELECTBOX1]->setCaption("Draw");
			mButtonText[BUTTON_SELECTBOX2]->setCaption("");
			mButtonText[BUTTON_SELECTBOX3]->setCaption("");
			mButtonText[BUTTON_SELECTBOX4]->setCaption("");
		}
		else if (isPet)
		{
			...
			if (tCritter->getCritterType() == "Manta")
				mButtonText[BUTTON_SELECTBOX4]->setCaption(tFlags->shrink ? "Devolve" : "Evolve");
			else
				mButtonText[BUTTON_SELECTBOX4]->setCaption("");
		}
		else
		...
		// === Левый клик на Critter ===
		if (!isRightClick)
		{
			if (!tCritter->getIsDead() && !isCraftStation && !isDrawPoint)
				mUnitManager->setPlayerTarget(tCritter);
		}
		else
		{
			// Правый клик: сразу открываем GUI для Crafting Station или Draw Point
			if (isCraftStation) showCraftGUI(tCritter);
			else if (isDrawPoint)
...
void MagixGUI::processSelectBoxClick()
{
	...
	if (tFunction == "Craft")
	{
		...
		{
			showCraftGUI(craftCritter); // открываем GUI крафта
			showSelectPanel(false);     // скрываем Select Panel
			return;
		}
	}
	// === КОНЕЦ КРАФТ СИСТЕМЫ ===
	...
	else if (tFunction == "Praise" || tFunction == "Pet")
	{
		...
		if (tFunction == "Praise" && tCritter->isInvulnerable()) mUnitManager->rewardCritter(tCritter);
	}
	else if (tFunction == "Stay" || tFunction == "Follow" || tFunction == "Shrink" || tFunction == "Grow" ||
		tFunction == "Evolve" || tFunction == "Devolve")
	{
		MagixCritter* tCritter = mCritterManager->getByObjectNode(mCurrentObject);
		if (!tCritter) return;

		if (tFunction == "Stay") mUnitManager->getPlayer()->getPetFlags()->stay = true;
		else if (tFunction == "Follow") mUnitManager->getPlayer()->getPetFlags()->stay = false;
...
void MagixGUI::unequipItem(const unsigned short &iID, bool doDrop)
{
	...
	else
	{
		...
		if (doDrop)
		{
			Vector3 playerPos = mUnitManager->getPlayer()->getPosition();
			mUnitManager->createItem(
				mItemManager->getNextEmptyID(mUnitManager->getPlayer()->getIndex() * 100),
				tEquip,
				playerPos.x + Math::RangeRandom(-2, 2),
				playerPos.z + Math::RangeRandom(-2, 2)
				);
		}
	}
}
...
void MagixGUI::dropSelectedStashItem()
{
	if (listButtonData == "")
	{
		mChatManager->message("Select an item from stash first.");
		return;
	}

	// Находим предмет по имени, а не по индексу
	const vector<const String>::type tStash = mItemManager->getStash();
	int actualLine = -1;

	// Ищем слот с таким же именем предмета
	for (int i = 0; i < (int)tStash.size(); i++)
	{
		if (tStash[i] == listButtonData)
		{
			actualLine = i;
			// Если нашли нужный слот (по индексу из listButtonLine), используем его
			if (i == listButtonLine)
			{
				break;
			}
	...
	if (actualLine == -1)
	{
		mChatManager->message("Item not found in stash.");
		return;
	}

	const String tItem = listButtonData;

	if (tItem != "")
	{
		mChatManager->message("Dropped " + mDef->getItemName(tItem) + " from stash.");

		if (!mGameStateManager->isCampaign())
		{
			mNetworkManager->sendItemDropFromStash(actualLine);
		}
		else
		{
			Vector3 playerPos = mUnitManager->getPlayer()->getPosition();
			Vector3 dropPos = playerPos + Vector3(
				Math::RangeRandom(-2, 2),
				0,
				Math::RangeRandom(-2, 2)
				);
			dropPos.y = mUnitManager->getGroundHeight(dropPos.x, dropPos.z);

			mItemManager->createItem(
				mItemManager->getNextEmptyID(0),
				tItem,
				dropPos
				);

			mItemManager->removeStashItem(actualLine);
		}
...
void MagixGUI::updateEmojis(Real timeSinceLastFrame)
{
	bool chatVisible = mBox[GUI_TEXTOUTPUT] && mBox[GUI_TEXTOUTPUT]->isVisible();

	for (auto it = mActiveEmojis.begin(); it != mActiveEmojis.end();) {
		it->second -= timeSinceLastFrame;

		if (it->first) {
			if (chatVisible && it->second > 0) {
				it->first->show();
			}
			else {
				it->first->hide();
		...
		if (it->second <= 0 || !chatVisible) {
			if (it->first) {
				it->first->hide();
				Overlay* chatOverlay = OverlayManager::getSingleton().getByName("GUIOverlay/Chat");
				if (chatOverlay) {
					chatOverlay->remove2D(static_cast<OverlayContainer*>(it->first));
				}
				OverlayManager::getSingleton().destroyOverlayElement(it->first);
			}
			it = mActiveEmojis.erase(it);
		}
		else {
			++it;
		}
