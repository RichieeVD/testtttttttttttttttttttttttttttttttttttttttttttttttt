...
void MagixUnitManager::initialize(SceneManager *sceneMgr, MagixExternalDefinitions *def, MagixGameStateManager *gameStateMgr, MagixWorld *world, MagixEffectsManager *effectsMgr, MagixCollisionManager *collisionMgr, MagixSoundManager *soundMgr, MagixItemManager *itemMgr, MagixCritterManager *critterMgr, MagixCamera *camera)
{
	...
	mModTagAttr = new MovableTextOverlayAttributes("ModNameAttrs", sceneMgr->getCamera("PlayerCam"), "Tahoma", 21, ModName);
	mAdminTagAttr = new MovableTextOverlayAttributes("AdminNameAttrs", sceneMgr->getCamera("PlayerCam"), AdmNameFont, 21, AdmName);
	mNameTagAttr = new MovableTextOverlayAttributes("Attrs1", sceneMgr->getCamera("PlayerCam"), "Tahoma", 20, ColourValue::White);
	mChatBubbleAttr = new MovableTextOverlayAttributes("Attrs2", sceneMgr->getCamera("PlayerCam"), "Tahoma", 19, ColourValue(1, 0.6, 0), "GUIMat/StatsBlockCenter");
	mUserTagAttr = new MovableTextOverlayAttributes("Attrs3", sceneMgr->getCamera("PlayerCam"), "Tahoma", 18, ColourValue(1, 0.6, 0));
	mPlayer->setEffectsManager(mEffectsManager);
}
...
bool MagixUnitManager::equipItem(MagixUnit *unit, const String &meshName, const short &slot)
{
	...
	if (mDef->getItemParticle(meshName) != "")
	{
		...
		// ÎÁÍÎÂËÅÍÍÀß ËÎÃÈÊÀ ÎÏÐÅÄÅËÅÍÈß ÝÍÒÈÒÈ ÄËß ÊÎÑÒÅÉ:
		Entity* targetEnt = 0;

		if (StringUtil::startsWith(tBone, "Head"))
			targetEnt = unit->getHeadEnt();
		else if (StringUtil::startsWith(tBone, "Tail"))
			targetEnt = unit->getTailEnt();
		else if (StringUtil::startsWith(tBone, "Arm4") ||  // ÄÎÁÀÂËÅÍÎ: êîñòè ïåðåäíèõ ëàï
			StringUtil::startsWith(tBone, "Leg4") ||  // ÄÎÁÀÂËÅÍÎ: êîñòè çàäíèõ ëàï
			StringUtil::startsWith(tBone, "Paw") ||
			StringUtil::startsWith(tBone, "Foot") ||
			StringUtil::startsWith(tBone, "Hand"))
			targetEnt = unit->getBodyEnt();  // Âñå êîñòè ëàï ïðèêðåïëÿþòñÿ ê òåëó
		else
			targetEnt = unit->getBodyEnt();  // Ïî óìîë÷àíèþ - òåëî

		if (targetEnt)
		{
			// ÑÎÇÄÀÅÌ ÓÍÈÊÀËÜÍÛÉ ÈÄÅÍÒÈÔÈÊÀÒÎÐ äëÿ ýôôåêòà
			String uniqueEffectID = "ItemEffect_" +
				StringConverter::toString(unit->getUnitID()) + "_" +
				StringConverter::toString(slot) + "_" +
				meshName + "_" + tBone;  // ÄÎÁÀÂËßÅÌ ÊÎÑÒÜ Â ID!

			printf("EQUIP: Creating effect %s attached to bone %s for slot %d\n",
				uniqueEffectID.c_str(), tBone.c_str(), slot);

			mEffectsManager->createItemEffect(
				targetEnt->getParentSceneNode(),
				mDef->getItemParticle(meshName),
				tBone,
				tOffsetAndOnNode.first,
				unit->getObjectNode()->getScale().x,
				uniqueEffectID
				);
		}
	}
...
{
	const String tItem = unit->getEquip(iID);

	printf("=== UNEQUIP START ===\n");
	printf("Slot: %d, Item: %s\n", iID, tItem.c_str());

	if (tItem != "")
	{
		const String tItemEffect = mDef->getItemParticle(tItem);
		const String tBone = mDef->getItemBone(tItem);

		printf("Item effect: %s, Bone: %s\n", tItemEffect.c_str(), tBone.c_str());

		if (tItemEffect != "")
		{
			String uniqueEffectID = "ItemEffect_" +
				StringConverter::toString(unit->getUnitID()) + "_" +
				StringConverter::toString(iID) + "_" +
				tItem + "_" + tBone;

			printf("Calling destroyItemEffectByUniqueID with: %s\n", uniqueEffectID.c_str());

			mEffectsManager->destroyItemEffectByUniqueID(uniqueEffectID);
		}
	}

	const String result = unit->unequip(mSceneMgr, mDef, iID);
	printf("=== UNEQUIP END ===\n");
	return result;
}
